...
class Form_liters(QWidget, Ui_liters_form):
    def __init__(self, main_window):
        ...
        self.liters = buckets_liters.copy()  # Инициализация ведер из глобальной переменной

        # Заполняем начальные значения ведер при первом запуске
        self.init_liters()

        self.btn_ok.clicked.connect(self.apply_liters)  # Применить изменения и закрыть окно
        self.btn_otmena.clicked.connect(self.cancel)  # Кнопка отмены

    def init_liters(self):
        """Инициализирует значения ведер при первом запуске программы."""
        for i in range(10):
            bucket_number, bucket_liters = self.liters[i]
            self.liter_boxes[i].setValue(bucket_liters)  # Устанавливаем начальное значение для каждого ведра

    def apply_liters(self):
        """Применяет изменения литров ведер и обновляет глобальную переменную buckets_liters."""
        for i in range(10):
            # Получаем введенные значения и обновляем глобальную переменную
            bucket_number = i + 1
            bucket_liters = self.liter_boxes[i].value()
            buckets_liters[i][1] = bucket_liters

        print(f"Applied liters: {buckets_liters}")  # Debug output

        self.main_window.update_buckets_from_liters()  # Обновляем отображаемые значения ведер в главном окне
        self.main_window.show()  # Показываем главное окно
        self.close()  # Закрываем текущее окно

    def cancel(self):
        """Отмена изменений, возвращаемся к предыдущим значениям."""
        self.main_window.show()  # Показываем главное окно
        self.close()  # Закрываем текущее окно
...
class Main_window(QMainWindow, Ui_MainWindow):
    ...
    def update_buckets_from_liters(self):
        """Обновляем количество литров в каждом ведре на основе глобальной переменной buckets_liters."""
        for i in range(10):
            self.Buckets.buckets[i][1] = buckets_liters[i][1]  # Обновляем количество литров для каждого ведра
